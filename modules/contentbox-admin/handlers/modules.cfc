...
component extends="baseHandler"{
	...
	function execute( event, rc, prc ){
		...
		if( isNull( module ) ){
			getModel( "messagebox@cbMessagebox" ).warn( "No modules where found with the following entryPoint: #rc.moduleEntryPoint#. Please make sure your module has an entry point." );
			return setNextEvent( prc.xehModules );
		}
		if( !module.isLoaded() ){
			getModel( "messagebox@cbMessagebox" ).warn( "The requested module: #rc.moduleEntryPoint# is not valid!" );
			return setNextEvent( prc.xehModules );
		}
		if( !module.getIsActive() ){
			getModel( "messagebox@cbMessagebox" ).warn( "The requested module: #rc.moduleEntryPoint# is not active!" );
			return setNextEvent( prc.xehModules );
		}
		if( !len(rc.moduleHandler) ){
			getModel( "messagebox@cbMessagebox" ).warn( "The requested module: #rc.moduleEntryPoint# is valid but the incoming module handler is empty!" );
			return setNextEvent( prc.xehModules );
		}
	...
	function activate( event, rc, prc ){
		...
		getModel( "messagebox@cbMessagebox" ).info( "Modules Activated, woohoo!" );
	...
	function deactivate( event, rc, prc ){
		...
		getModel( "messagebox@cbMessagebox" ).info( "Modules Deactivated!" );
		setNextEvent(prc.xehModules);
	}
	...
	function reset( event, rc, prc ){
		...
		getModel( "messagebox@cbMessagebox" ).info( "Modules Reset!" );
		setNextEvent(prc.xehModules);
	}
	...
	function rescan( event, rc, prc ){
		...
		getModel( "messagebox@cbMessagebox" ).info( "Modules Rescaned and Revamped!" );
		setNextEvent(prc.xehModules);
	}
	...
	function remove( event, rc, prc ){
		...
		getModel( "messagebox@cbMessagebox" ).info( "Module Removed Forever!" );
		setNextEvent(prc.xehModules);
	}
	...
	function upload( event, rc, prc ){
		...
		// Verify
		if( len( fp ) eq 0){
			getModel( "messagebox@cbMessagebox" ).warn( "Please choose a file to upload" );
		}
		else{
			// Upload File
			try{
				...
				if( results.error ){
					...
					getModel( "messagebox@cbMessagebox" ).error( "Error installing module, please check out the log information." );
				}
				else{
					// Messagebox
					getModel( "messagebox@cbMessagebox" ).info( "Module Installed Successfully in your 'modules' folder." );
				...
				getModel( "messagebox@cbMessagebox" ).error( "Error Installing Module: #e.detail# #e.message#" );
			}
		}
