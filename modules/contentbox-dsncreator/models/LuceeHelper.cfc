...
<cfcomponent output="false" hint="ContentBox DSN creator helper" extends="BaseHelper">
	...
	<!--- verifyCFMLAdmin --->
	<cffunction
		name      ="verifyCFMLAdmin"
		output    ="false"
		access    ="public"
		returntype="struct"
		hint      ="Verify if the cf admin password validates. Returns struct: {error:boolean, messages:string}"
	>
		<cfargument name="cfmlPassword" required=true>
		<cfscript>
		var results = { "ERROR" : false, "MESSAGES" : "" };
		try {
			var isValid = isValidLuceePassword( arguments.cfmlPassword );
	...
	<cffunction
		...
		access    ="public"
		returntype="any"
		hint      ="Create the DSN, returns struct: {error:boolean, messages:string}"
	>
		<cfargument name="cfmlPassword" required=true>
		...
		<cftry>
			...
			<cfif listFindNoCase( valueList( local.datasources.name ), arguments.dsnName )>
				<cfthrow
					type   ="DuplicateDSNException"
					message="Datsource #arguments.dsnName# already exists!"
				>
			</cfif>
			...
			<cfswitch expression="#arguments.dbType#">
				...
				<cfcase value="mysql">
					...
					<cfset local.className = "com.mysql.cj.jdbc.Driver">
				</cfcase>
				<cfcase value="oracle">
	...
	<!--- isValidLuceePassword --->
	<cffunction
		name      ="isValidLuceePassword"
		output    ="false"
		access    ="private"
		returntype="boolean"
		hint      ="Validates Railo Admin password"
	>
		<cfargument name="cfmlPassword" required=true>

		<cftry>
			<cfadmin action="connect" type="web" password="#arguments.cfmlPassword#">
			<cfreturn true>
			<cfcatch type="any">
				<cfreturn false>
			</cfcatch>
