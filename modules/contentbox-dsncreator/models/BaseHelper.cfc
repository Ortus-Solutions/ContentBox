...
<cfcomponent output="false" hint="ContentBox DSN Base helper">
	...
	<cffunction
		...
		access    ="public"
		returntype="struct"
		hint      ="Verify the DSN exists, returns struct: {error:boolean, exists:boolean, messages:string}"
	>
	...
	<cffunction
		...
		access    ="public"
		returntype="any"
		hint      ="Update the application's DSN and data"
	>
		<cfargument name="dsnName" required="true"/>
		<cfscript>
		...
		c = replaceNoCase(
			...
			"this.datasource = ""#arguments.dsnName#"""
		);
		// Update relocations
		c = replaceNoCase(
			c,
