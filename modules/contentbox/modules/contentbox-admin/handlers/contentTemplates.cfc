component extends="baseHandler" {
	...
	function exportAll( event, rc, prc ){
		...
		return templateService.newCriteria()
					.isIn( "templateID", listToArray( rc.templateID ) )
					.list()
					.map( function( template ){ return template.getMemento( profile="export" ) } );
	...
	function importAll( event, rc, prc ){
		...
		try {
			if ( len( rc.importFile ) and fileExists( rc.importFile ) ) {
				var templates = deSerializeJSON( fileRead( rc.importFile ) )
								.map( function( template ){
									param template.creator = prc.oCurrentAuthor;
									param template.site = prc.oCurrentSite;
									// Population arguments
									if( rc.overrideContent ){
										var templateEntity = templateService.newCriteria().isEq( "site", prc.oCurrentSite ).isEq( "name", template.name ).get();
				...
				cbMessagebox.info( "Templates #templates.map( function( t ){ return t.getName(); } ).toList()# are now associated to site #prc.oCurrentSite.getName()#" );
			} else {
