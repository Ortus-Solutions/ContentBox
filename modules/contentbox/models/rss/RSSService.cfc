...
component singleton{
	...
	RSSService function clearCaches( boolean comments=false, string slug="" ){
		...
		if( variables.log.canInfo() ){
			variables.log.info( "Sent clear command using the following content key: #cacheKey# from provider: #variables.cache.getName()#" );
		}
	...
	function getRSS(
		...
		if( settings.cb_rss_caching ){
			variables.cache.set(cacheKey, rssFeed, settings.cb_rss_cachingTimeout, settings.cb_rss_cachingTimeoutIdle);
		}
	...
	private function buildEntryFeed( string category = "", string siteId = "" ){
		...
		for(var i = 1; i lte entryResults.count; i++){
			...
			qEntries.linkComments[i]  = variables.CBHelper.linkComments( entryResults.entries[i] );
