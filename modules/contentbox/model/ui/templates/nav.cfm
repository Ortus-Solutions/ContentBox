...
<cfloop array="#local.thisMenu#" index="local.topMenu">
	...
	<li class="sub-menu<cfif event.getValue(name='tab#local.topMenu.name#',defaultValue=false,private=true)> active</cfif>#local.topMenu.class#" data-name="#local.topMenu.name#">
		...
		----><cfif structKeyExists( local.topMenu, "data" ) && structCount( local.topMenu.data )>#parseADataAttributes( local.topMenu.data )#</cfif>>#( isCustomFunction( local.topMenu.label ) ? local.topMenu.label() : local.topMenu.label )# <cfif arrayLen( local.topMenu.subMenu )><i class="arrow fa fa-angle-right pull-right"></i></cfif></a>
		<!--- Do we have submenus --->
