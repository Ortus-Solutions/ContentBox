/**
* Detects mobile device browsers
*/
component singleton="true"{
	...
	MobileDetector function init(){
		...
		// Plugin Properties
		setPluginName("MobileDetector");
		setPluginVersion("1.0");
		setPluginDescription("Detects mobile device browsers");
	...
	boolean function isMobile(){
		if( reFindNoCase("android.+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|meego.+mobile|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino", CGI.HTTP_USER_AGENT) GT 0 OR 
