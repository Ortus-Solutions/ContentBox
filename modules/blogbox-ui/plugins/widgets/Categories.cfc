...
component extends="blogbox.model.ui.BaseWidget" singleton{
	...
	*/
	any function renderIt(boolean dropdown=false,boolean showPostCount=true,title="",titleLevel="2"){
		var categories 		= bb.getCurrentCategories();
		var rString			= "";
	...
	private function buildDropDown(categories,showPostCount){
		...
		saveContent variable="rString"{
			...
			for(var x=1; x lte arrayLen( arguments.categories ); x++){
				writeOutput('<option value="#bb.linkCategory(arguments.categories[x])#">#arguments.categories[x].getCategory()#');
	...
	private function buildList(categories,showPostCount){
		...
		saveContent variable="rString"{
			...
			// iterate and create
			for(var x=1; x lte arrayLen( arguments.categories ); x++){
				writeOutput('<li class="categories"><a href="#bb.linkCategory(arguments.categories[x])#">#arguments.categories[x].getCategory()#');
				if( arguments.showPostCount ){ writeOutput(" (#arguments.categories[x].getNumberOfEntries()#)"); }
				writeOutput('</a></li>');
			}
			// close ul
