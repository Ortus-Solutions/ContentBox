...
component {
	...
	void function postEvent(event,struct interceptData) eventPattern="^blogbox-ui:blog\.(page|entry)"{
		...
		if( structKeyExists(prc,"entry") ){ content = prc.entry; }
		...
		if( !securityService.isContentViewable( content ) OR len( event.getHTTPHeader("Authorization","") ) ){
			...
			event.setHTTPHeader(name="WWW-Authenticate",value="basic realm=""BlogBox content protection, enter the content password""");
